...
class SpaceOnlyEnv(gym.Env):
    """Minimal env to satisfy SB3 model initialization with correct spaces."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train phase-2 hierarchical PPO with 3 SB3 agents")
    ...
    p.add_argument("--timesteps-per-iter", type=int, default=1024)
...
def make_model(obs_space: gym.Space, action_space: gym.Space, seed: int, n_steps: int, log_name: str) -> PPO:
    def make_env():
        return SpaceOnlyEnv(obs_space, action_space)
    ...
    model.set_logger(configure(folder=os.path.join("/tmp", "sb3_multi_logs", log_name), format_strings=["stdout"]))
    return model


def collect_rollouts(
    env: MECPhase2HierarchicalEnv,
    models: Dict[str, PPO],
    rollout_steps: int,
) -> Tuple[Dict[str, RolloutBuffer], Dict[str, np.ndarray], bool]:
    obs, _ = env.reset()
    episode_start = {aid: True for aid in obs}
    last_done = False
    ...
    for _ in range(rollout_steps):
        ...
        for aid, model in models.items():
            obs_tensor, _ = model.policy.obs_to_tensor(obs[aid])
            with torch.no_grad():
                action_tensor, value, log_prob = model.policy.forward(obs_tensor, deterministic=False)
            action = action_tensor.cpu().numpy()
            actions[aid] = np.asarray(action, dtype=np.float32).reshape(-1)
            values[aid] = value
        ...
        for aid, model in models.items():
            obs_np = np.asarray(obs[aid], dtype=np.float32).reshape(1, -1)
            act_np = np.asarray(actions[aid], dtype=np.float32).reshape(1, -1)
            model.rollout_buffer.add(
                obs_np,
                act_np,
                np.array([float(rewards[aid])], dtype=np.float32),
                np.array([episode_start[aid]], dtype=bool),
                values[aid],
                log_probs[aid],
            )
            episode_start[aid] = done

        obs = next_obs
        if done:
            obs, _ = env.reset()

    return {aid: model.rollout_buffer for aid, model in models.items()}, obs, last_done


def train_models(models: Dict[str, PPO], last_obs: Dict[str, np.ndarray], last_done: bool) -> None:
    for aid, model in models.items():
        obs_tensor, _ = model.policy.obs_to_tensor(last_obs[aid])
        with torch.no_grad():
            last_values = model.policy.predict_values(obs_tensor)
        model.rollout_buffer.compute_returns_and_advantage(
            last_values, np.array([last_done], dtype=bool)
        )
        model.train()


def evaluate_episode(models: Dict[str, PPO], env_cfg: Dict[str, Any], eval_seed: int) -> Dict[str, Any]:
    env = MECPhase2HierarchicalEnv({**env_cfg, "seed": eval_seed, "aggregate_agents": True})
    obs, _ = env.reset()
    ...
    while not done:
        actions: Dict[str, np.ndarray] = {}
        for aid, model in models.items():
            action, _ = model.predict(obs[aid], deterministic=True)
            actions[aid] = action
...
def evaluate_mean(models: Dict[str, PPO], env_cfg: Dict[str, Any], base_seed: int, n_episodes: int = 5) -> Dict[str, Any]:
    stats = [evaluate_episode(models, env_cfg, eval_seed=base_seed + 1000 + i) for i in range(n_episodes)]
...
def run_for_k(k: int, args: argparse.Namespace) -> None:
    ...
    env = MECPhase2HierarchicalEnv(env_cfg)
    models = {
        "coordinator": make_model(
            env.coordinator_obs_space,
            env.coordinator_action_space,
            args.seed + 1,
            args.timesteps_per_iter,
            log_name=f"coordinator_k{k}",
        ),
        "ue": make_model(
            env.ue_joint_obs_space,
            env.ue_joint_action_space,
            args.seed + 2,
            args.timesteps_per_iter,
            log_name=f"ue_k{k}",
        ),
        "cluster": make_model(
            env.cluster_joint_obs_space,
            env.cluster_joint_action_space,
            args.seed + 3,
            args.timesteps_per_iter,
            log_name=f"cluster_k{k}",
        ),
    }
    ...
    for it in range(1, args.iterations + 1):
        _, last_obs, last_done = collect_rollouts(env, models, args.timesteps_per_iter)
        train_models(models, last_obs, last_done)

        stats = evaluate_mean(models, env_cfg, base_seed=10000 + k * 100 + it, n_episodes=5)
        ...
        if score > best_score:
            best_score = score
            for aid, model in models.items():
                model.save(os.path.join(best_dir, f"{aid}_ppo_model"))
    ...
    for aid, model in models.items():
        model.save(os.path.join(out_dir, f"{aid}_ppo_model"))
